...
# ╔═╡ b6d4cc3f-ff05-4c48-bb55-68765f635de1
TEXT_HEIGHT = Dict(:beamer => 244u"pt")

# ╔═╡ a8b835c8-9983-4edb-8b83-daca277c065b
MM.Themes.width!(TEXT_WIDTH[:report])

# ╔═╡ 1460901f-e7e4-4c20-b3f2-15d1ac9c4820
MM.export_format!(:pdf)
...
MM.Themes.update_theme!(:base, 
	MM.Themes.get_theme(:base, theme_latexfonts(),
		Theme(
			Axis=(;
				xticksize=2.5, yticksize=2.5, # half default
		    	xgridvisible=true, ygridvisible=true
			),
			Colorbar=(;
				ticksize=2.5 # half default
			),
			Legend=(;
				rowgap=1.5, # half default
				colgap=8, # half deafult
				backgroundcolor=:white, framevisible=true, framewidth=1,
...
# ╔═╡ eac8aa73-f660-4825-9a3a-fbbe0040c804
MM.Themes.update_theme(:report, Theme(;
    fontsize=10.95,
    Axis=(;
		titlesize=12, # large
        xlabelsize=10, ylabelsize=10, # small
        xticklabelsize=8, yticklabelsize=8, # scriptsize
	),
	Colorbar=(;
		ticklabelsize=8 # scriptsize
	),
	Legend=(;
		labelsize=8 # scriptsize
	)
));

# ╔═╡ b3e218e2-01bc-40f7-8c20-1158cacf7223
MM.Themes.update_theme!(:slides, MM.Themes.get_theme(:report, 
		Theme(;
			Lines=(;linewidth=0.5),
			Scatter=(;markersize=1),
			ScatterLines=(;linewidth=0.5),
			Legend=(;
				padding=(3,3,3,3),
				patchlabelgap=1,
				patchsize=(10,10)
...
# ╔═╡ 0324f5ca-0b00-461f-bf69-9d08879dbcc7
MM.Themes.theme_keys()
...
begin
    savefig(fig_singular_values, (16,), TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_singular_values, (16,), "fig_singular_values_slides", ( 0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
savefig(fig_heatmap_singular_vectors, (100,), TEXT_WIDTH[:report], 0.6; override_theme=MM.Themes.get_theme(:report))

# ╔═╡ 27d0f237-4f24-4ac2-891a-3e965ae41823
function fig_compare_singular_vectors(n=3; legend=true)
    f = Figure()
	rowgap!(f.layout, 0)
    ax = Axis(f[1, 1])
    ax.xlabel = L"t"
...
begin
    savefig(fig_compare_singular_vectors, (3,), "fig_singular_vectors", TEXT_WIDTH[:report] / 2, 1; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_compare_singular_vectors, (3,), "fig_singular_vectors_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), legend=false)
end
...
#=╠═╡
begin
    savefig(fig_sum_singular_demo, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_sum_singular_demo, "fig_sum_singular_demo_slides",  (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides, Theme(;
	Lines=(;linewidth=0.1),
	Scatter=(;markersize=0.2)
	)))
end
  ╠═╡ =#
...
#=╠═╡
function fig_singular_demo(inds=[2, 4, 6]; markersize=4, legend=true)
    f = Figure()
    ax = Axis(f[1, 1])
    ax.xlabel = L"t"
    ax.xticks = ([0, π / 2, π], [L"0", L"\pi/2", L"\pi"])
    for (i, clr) in zip(inds, Makie.wong_colors())
        ...
        scatter!(ax, ts, A(0.05, M, N) * sing.(ss), markersize=markersize, color=clr, label=latexstring(raw"A U_{\cdot" * "$i}"))
    end
	if legend
    	Legend(f[1, 2], ax, nbanks=3, margin=(0, 0, 0, 0), tellwidth=false)
	end
	xlims!(ax, 0 - 0.05, π + 0.05)
    rowgap!(f.layout, 1)
    f
...
#=╠═╡
begin
    savefig(fig_singular_demo, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_singular_demo, "fig_singular_demo_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides, ), markersize=2, legend=false)
end
  ╠═╡ =#
...
function fig_fixed_function_noised(;markersize=2)
    f, ax, _ = lines(0 .. π, x, label=L"x")
    scatterlines!(ss, y_rec; color=:red, label=L"Kx", marker=:x, markersize=markersize)
...
begin
    savefig(fig_fixed_function_noised, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_fixed_function_noised, "fig_fixed_function_noised_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=2.5)
end
...
function fig_picard_plot(ran=1:40; markersize=5, labelsize=12)
    f, ax, _ = scatter(ran, abs.(y_coeff[ran]), axis=(; yscale=log10), label=L"|(y^\delta, y_j\,)|";markersize)
    ax.xlabel = L"j"
    scatter!(ax, ran, abs.(y_coeff[ran]) ./ μj.(ran), label=L"{|(y^\delta, y_j)|}/{\mu_j}"; markersize)
    # scatter!(ax, ran, abs.((U_A'*y_rec)[ran]), label=L"|(y, y_j)|"; markersize)
	scatter!(ax, ran, μj.(ran), label=L"\mu_j"; markersize)
    # axislegend(ax, position=:lt)
    axislegend(ax; position=:lt)
	ylims!(eps(), 10.0^35)
    xlims!(0.5, 40.5)
...
begin
    savefig(fig_picard_plot, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_picard_plot, "fig_picard_plot_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=5)
end
...
begin
    savefig(fig_abel_intro, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_intro,"fig_abel_intro_slides", (0.9TEXT_HEIGHT[:beamer], TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
begin
    savefig(fig_abel_TSVD, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_TSVD,"fig_abel_TSVD_slides",  (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
function fig_abel_TSVD_relative_err(;markersize=4)
    f, ax, _ = lines(1:100, k -> norm(x2.(ts2) .- x_rec_TSVD(k)) / norm(x2.(ts2)); label=L"\Vert x - x_k \,\Vert\, /\, \Vert x\, \Vert")
    ax.xlabel = L"k"
    scatter!(opt_k, norm(x2.(ts2) .- x_rec_TSVD(opt_k)) / norm(x2.(ts2)); color=:red, label=L"\min_k\{\Vert x - x_k \,\Vert\, /\, \Vert x\, \Vert\}", markersize)
    axislegend(ax; position=:rt)
    xlims!(ax, 0, 101)
    f
end
...
begin
    savefig(fig_abel_TSVD_relative_err, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_TSVD_relative_err, "fig_abel_TSVD_relative_err_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=4)
end
...
begin
    savefig(fig_abel_Tikhonov, (σ2 .* [3, 7, 10],), TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_Tikhonov, (σ2 .* [3, 7, 10],), "fig_abel_Tikhonov_slides",  (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
function fig_abel_Tikhonov_relative_err(;markersize=4)
    f, ax, _ = lines((3σ2) .. (10σ2), α -> norm(x2.(ts2) .- x_rec_Tikhonov(α)) / norm(x2.(ts2)); label=L"\Vert x - x_α \,\Vert\, /\, \Vert x\, \Vert")
    ax.xlabel = L"α"
    scatter!(Optim.minimizer(opt_α)[1], minimum(opt_α); label=L"\min_α\{\Vert x - x_α \,\Vert\, /\, \Vert x\, \Vert\}", color=:red, markersize)
...
begin
    savefig(fig_abel_Tikhonov_relative_err, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_Tikhonov_relative_err, "fig_abel_Tikhonov_relative_err_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=4)
end
...
begin
    savefig(fig_abel_compare, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_compare,"fig_abel_compare_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
begin
    savefig(fig_abel_discont_intro, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_intro,"fig_abel_discont_intro_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
begin
    savefig(fig_abel_discont_TSVD, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_TSVD,"fig_abel_discont_TSVD_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
function fig_abel_discont_TSVD_relative_err(;markersize=4)
    f, ax, _ = lines(1:100, k -> norm(x3.(ts2) .- x_rec_TSVD_3(k)) / norm(x3.(ts2)); label=L"\Vert x - x_k \,\Vert\, /\, \Vert x\, \Vert")
    ax.xlabel = L"k"
    scatter!(opt_k_3, norm(x3.(ts2) .- x_rec_TSVD_3(opt_k_3)) / norm(x3.(ts2)); color=:red, label=L"\min_k\{\Vert x - x_k \,\Vert\, /\, \Vert x\, \Vert\}", markersize)
    axislegend(ax; position=:rt)
    xlims!(ax, 0, 101)
    f
end
...
begin
    savefig(fig_abel_discont_TSVD_relative_err, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_TSVD_relative_err,"fig_abel_discont_TSVD_relative_err_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=4)
end
...
begin
    savefig(fig_abel_discont_Tikhonov, (σ3 .* [3, 7, 10],), TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_Tikhonov, (σ3 .* [3, 7, 10],),"fig_abel_discont_Tikhonov_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
function fig_abel_discont_Tikhonov_relative_err(;markersize=4)
    f, ax, _ = lines((3σ3) .. (10σ3), α -> norm(x3.(ts2) .- x_rec_Tikhonov_3(α)) / norm(x3.(ts2)); label=L"\Vert x - x_α \,\Vert\, /\, \Vert x\, \Vert")
    ax.xlabel = L"α"
    scatter!(Optim.minimizer(opt_α_3)[1], minimum(opt_α_3); label=L"\min_α\{\Vert x - x_α \,\Vert\, /\, \Vert x\, \Vert\}", color=:red, markersize)
    axislegend(ax; position=:ct)
...
begin
    savefig(fig_abel_discont_Tikhonov_relative_err, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_Tikhonov_relative_err,"fig_abel_discont_Tikhonov_relative_err_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides), markersize=4)
end
...
begin
    savefig(fig_abel_discont_compare, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_abel_discont_compare,"fig_abel_discont_compare_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
function fig_limit_demo(;legend=true)
    f, ax, _ = lines(ts4, x4; label=L"x")
    lines!(ts4, y_rec4; label=L"y")
...
begin
    savefig(fig_limit_demo, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_limit_demo, "fig_limit_demo_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides))
end
...
begin
    savefig(fig_numerical_limit, TEXT_WIDTH[:report] / 2; override_theme=MM.Themes.get_theme(:report))
    savefig(fig_numerical_limit, "fig_numerical_limit_slides", (0.6TEXT_HEIGHT[:beamer], 0.8TEXT_WIDTH[:beamer]); override_theme=MM.Themes.get_theme(:slides, Theme(;figure_padding=5)))
end
...
manifest_format = "2.0"
project_hash = "bd883823c771bd716ca6f8e7c36e70072f4a9e23"
